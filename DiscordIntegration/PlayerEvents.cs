					...
					else
					{
						...
						else if (!Plugin.Singleton.Config.OnlyFriendlyFireDMG)
						{
							...
							else
							{
								...
								ProcessSTT.SendData($"({ev.Attacker.Id}) {ev.Attacker.Nickname} - `{ev.Attacker.UserId}` ({ev.Attacker.Role}) {Plugin.Translation.Damaged} ({ev.Target.Id}) {ev.Target.Nickname} - `{ev.Target.UserId}` ({ev.Target.Role}) {Plugin.Translation._For} {ev.Amount} {Plugin.Translation.With} {DamageTypes.FromIndex(ev.Tool).name}.", HandleQueue.GameLogChannelId);
							}
						}
					}
				}
