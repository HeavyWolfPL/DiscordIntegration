    ...
    public class ServerEvents
    {
        ...
        public void OnCommand(SendingRemoteAdminCommandEventArgs ev)
        {
            ...
            if (Plugin.Singleton.Config.RaCommands)
                ProcessSTT.SendData($":keyboard: ({ev.Sender.Id}) {ev.Sender.Nickname} ({ev.Sender.UserId} - {ev.Sender.Role}) {Plugin.Translation.UsedCommand}: {ev.Name} {Args}", HandleQueue.CommandLogChannelId);
